...
class TestPMCEndpoint:
    ...
    async def test_pmc_with_date_range(self, auth_client):
        ...
        response = await auth_client.get(f"/api/pmc?start={start.isoformat()}&end={end.isoformat()}")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_pmc_with_activity_tss(self, auth_client, db_session):
        ...
        response = await auth_client.get("/api/pmc?start=2024-03-01&end=2024-03-31")
        assert response.status_code == 200
        data = response.json()

        # Find the day of the activity (March 15)
        day_of_activity = next((d for d in data if d["date"] == "2024-03-15"), None)
