...
def calculate_curvature(p1: tuple, p2: tuple, p3: tuple) -> float:
    ...
    Uses the Menger curvature formula: κ = 4A / (|p1-p2| × |p2-p3| × |p1-p3|)
...
def extract_descent_points(records: list[Record], activity_id: str) -> list[DescentPoint]:
    ...
    valid = [
        ...
        and r.speed_mps is not None and r.speed_mps > 2  # Moving
    ]
...
async def run_analysis(user_id: int | None = None) -> None:
    ...
    print(f"  Model B (grade + curvature):    R² = {results['r2_grade_curvature']:.3f} ({results['r2_grade_curvature']*100:.1f}%)")
    print(f"  Model C (+ interaction):        R² = {results['r2_with_interaction']:.3f} ({results['r2_with_interaction']*100:.1f}%)")
    ...
    if improvement < 0.02:
        ...
        print(f"  RECOMMENDATION: NOT worth modeling cornering behavior.")
        print(f"  Grade alone is sufficient for descent speed prediction.")
    elif improvement < 0.10:
        print(f"\n  Curvature explains {improvement*100:.1f}% additional variance.")
        print(f"  RECOMMENDATION: MARGINAL value. Consider if complexity is justified.")
    else:
        print(f"\n  Curvature explains {improvement*100:.1f}% additional variance.")
        print(f"  RECOMMENDATION: WORTH modeling cornering behavior.")
    
    print(f"\n  Regression coefficients (Model B):")
    print(f"    speed = {results['intercept']:.2f} + {results['grade_coefficient']:.3f}×grade + {results['curvature_coefficient']:.1f}×curvature")
