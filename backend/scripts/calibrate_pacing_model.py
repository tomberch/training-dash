...
from trainingdash.domain.pacing import calculate_normalized_power_from_segments
...
async def get_user_weight(user_id: int) -> float:
    ...
    async with async_session() as db:
        result = await db.execute(
            select(User.weight_kg).where(User.id == user_id)
        )
...
def create_segments_from_records(records: list[Record]) -> tuple[list[CourseSegment], str]:
    ...
    valid_records = [
        r for r in records
        if r.lat is not None and r.lon is not None and r.altitude_m is not None
    ]
...
def print_summary(results: list[ActivityResult]) -> None:
    ...
    # By course type
    print("\nBy Course Type:")
    course_types = set(r.course_type for r in successful)
    ...
    if outliers:
        ...
        for r in sorted(outliers, key=lambda x: -x.np_error_pct)[:5]:
            print(f"  {r.activity_title} ({r.activity_date}): {r.np_error_pct:.1f}% error, VI actual={r.actual_vi:.3f} vs pred={r.pred_vi:.3f}")
    ...
    if mountain_results:
        ...
        if abs(mountain_bias) > 0.03:
            print(f"  Consider adjusting steep climb coefficients")
