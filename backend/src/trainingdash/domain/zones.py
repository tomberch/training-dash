...
def _parse_custom_pct(custom_pct: dict[str, Any] | None) -> dict[int, tuple[int, int | None]] | None:
    ...
    if not custom_pct:
        return None
    try:
        return {int(k): (v[0], v[1] if v[1] is not None else None) for k, v in custom_pct.items()}
...
def compute_zone_times(
    ...
    if ftp and ftp > 0:
        power_zone_times = dict.fromkeys(range(1, 8), 0)
    ...
    if hr_data and lthr and lthr > 0:
        hr_zone_times = dict.fromkeys(range(1, 6), 0)
        for hr in hr_data:
