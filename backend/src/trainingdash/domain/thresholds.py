...
async def _get_metric_type_id(db: AsyncSession, key: str) -> int | None:
    ...
    result = await db.execute(select(MetricType.id).where(MetricType.key == key))
...
async def _get_metric_for_date(db: AsyncSession, user_id: int, metric_key: str, target_date: date) -> int | None:
...
async def get_ftp_for_date(db: AsyncSession, user_id: int, target_date: date) -> int | None:
    """Get FTP value effective on a given date."""
    return await _get_metric_for_date(db, user_id, "ftp", target_date)


async def get_lthr_for_date(db: AsyncSession, user_id: int, target_date: date) -> int | None:
    """Get LTHR value effective on a given date."""
    return await _get_metric_for_date(db, user_id, "lthr", target_date)


async def get_hrmax_for_date(db: AsyncSession, user_id: int, target_date: date) -> int | None:
    """Get HRmax value effective on a given date."""
    return await _get_metric_for_date(db, user_id, "hrmax", target_date)


async def get_thresholds_for_date(db: AsyncSession, user_id: int, target_date: date) -> ThresholdValues:
    """
...
async def get_all_threshold_entries(db: AsyncSession, user_id: int) -> list[dict]:
    """
...
async def create_threshold_entries(
    ...
    if ftp_watts is not None:
        entry = await create_metric_entry(db, user_id, "ftp", ftp_watts, effective_date, source, source_detail)
        if entry:
            entries.append(entry)

    if lthr_bpm is not None:
        entry = await create_metric_entry(db, user_id, "lthr", lthr_bpm, effective_date, source, source_detail)
        if entry:
            entries.append(entry)

    if hrmax_bpm is not None:
        entry = await create_metric_entry(db, user_id, "hrmax", hrmax_bpm, effective_date, source, source_detail)
        if entry:
            entries.append(entry)
...
async def get_threshold_for_date(db: AsyncSession, user_id: int, target_date: date) -> ThresholdValues | None:
    """
