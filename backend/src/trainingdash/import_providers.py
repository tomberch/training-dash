"""
Import provider implementations for Xert and Garmin.

These implement the ImportProvider interface from integrations/protocols.py,
handling provider-specific authentication and activity fetching while the
common orchestration logic lives in ImportFromProvider use case.
"""
...
from trainingdash.integrations.protocols import CredentialInfo, ImportProvider, ProviderActivity
from trainingdash.repositories.postgres.models import Activity, GarminCredentials, XertCredentials

logger = logging.getLogger(__name__)


class XertImportProvider(ImportProvider):
    """
    Xert import provider.
    ...
    async def list_activities(self, start_date: datetime, end_date: datetime) -> list[ProviderActivity]:
        ...
        activities = await self._client.list_activities(from_timestamp=from_ts, to_timestamp=to_ts)
    ...
    async def ingest_activity(
        ...
        try:
            ...
            ingested = await ingest_fit(db, user_id, fit_bytes, "xert", source_ref, batch_mode)
...
class GarminImportProvider(ImportProvider):
    """
    Garmin Connect import provider.
