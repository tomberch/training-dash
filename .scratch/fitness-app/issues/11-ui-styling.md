...
- [ ] Tailwind CSS is set up via `@tailwindcss/vite` and utility classes are used throughout
- [ ] Login screen is centered, styled, and readable
- [ ] Activity list is a clean table with hover states, proper spacing, and responsive layout
- [ ] Activity detail stat tiles are visually grouped (card or grid), not raw inline divs
- [ ] Charts have consistent styling — readable axes, clean tooltips, no overflow, consistent height/spacing
- [ ] Map has a sensible default zoom that fits the polyline, clean tile layer, no layout issues
- [ ] Axis toggle buttons look like toggle buttons, not raw text
- [ ] Upload form is styled (file input + feedback state)
- [ ] App is usable on tablet-width screens (responsive breakpoints)
